...
logname='./data/mera_sweep.log'
if not os.path.exists(os.path.dirname(logname)):
    os.makedirs(os.path.dirname(logname))
with redirect_stdout(Tee(logname)):
    for g in tqdm([0.5,1.5,0.8,1.2,0.9,1.1,0.95,1.05,0.98,1.02,0.99,1.01,0.995,1.005,0.998,1.002,0.999,1.001,1]):
        J=1
        hAB,hBA=get_Ising1D_Hamiltonians_2blocked(J,g)
        print('J=%f, g=%f'%(J,g))
        filename='./data/Ising_g%.4f_X8_L7.pth'%(g)
        ...
        optionss=[
            ...
            MERAOptions(nLayers=4,max_dim=8,max_dim_mid=6,nSweeps=1800),
            MERAOptions(nLayers=5,max_dim=8,max_dim_mid=6,nSweeps=1800),
            MERAOptions(nLayers=6,max_dim=8,max_dim_mid=6,nSweeps=1800),
            MERAOptions(nLayers=7,max_dim=8,max_dim_mid=6,nSweeps=1800),
        ]
